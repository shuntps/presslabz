...
`handlerTimeout` is **0 — off — by default**, and is the one timeout allowed to be zero. Fastify's is cooperative: it sends 503 and aborts `request.signal`, and the handler keeps running until something observes that signal. One place now does — the upload admission gate drops a queued upload the moment its client goes, which is safe precisely because a request still *waiting* for a turn has started nothing. That does not make the timeout safe for work already underway: Drizzle writes and an image pipeline in progress are still not cancelled, and — measured — a handler that ignores the signal receives its 503 and still completes its write. Turning the timeout on before cancellation is wired through to those would mean answering 503 while the database write it was meant to stop landed anyway, and the client retried. That wiring is named work, not a solved problem.
...
**`z.httpUrl()` is the obvious answer for an endpoint setting and cannot be used here.** Measured against Zod 4.4.3, it refuses `http://localhost:9000`, `http://127.0.0.1:9000` and `http://storage:9000` — every hostname without a public dotted suffix, which is development, Docker Compose, and any installation whose bucket is a service name on an internal network. The endpoint settings therefore check the *scheme* — `http:` or `https:`, so `ftp://`, `file://` and `javascript:` are refused — and leave the hostname alone. Following the recommendation literally would have broken every local install.
