...
**Attaching a translation is authorized, not merely addressed.** Opening a group needs create permission for the type; joining one needs create permission *and* update permission over at least one existing member. Read permission is never sufficient, and a group id is not a secret — it must never be the thing that grants access. An assigned-translator workflow, when it arrives, gets an explicit capability rather than a relaxation of this.
